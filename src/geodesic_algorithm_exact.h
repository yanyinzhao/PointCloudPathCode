...
namespace geodesic
{
	...
	private:
		...
		bool check_stop_conditions_distance();
		bool check_stop_conditions_cover_points(unsigned &index, int k = 100000000);
												  ...
												  int k)
	{
		...
		while (!m_queue.empty())
		{
			...
			if (++m_iterations % check_period == 0) // check if we covered all required vertices
			{
				if (check_stop_conditions_cover_points(satisfied_index, k))
				{
					break;
				}
			}
	...
	inline bool GeodesicAlgorithmExact::check_stop_conditions_distance()
	{
		double queue_distance = (*m_queue.begin())->min();
	...
	inline bool GeodesicAlgorithmExact::check_stop_conditions_cover_points(unsigned &index, int k)
	{
		double queue_distance = (*m_queue.begin())->min();
